...
jl_state_names <- function(species) {
  
  # Create the function string
  state.string <- "function state_names()\n\t[\n"
  
  for (i in seq_along(species)) {
    if (i != length(species)) {
      line <- paste("\t\t\"", species[i], "\",", sep = "")
...
jl_diff_equations <- function(species, 
  ...
  diff_eq_string <- paste(diff_eq_string, "\treturn d\nend", sep = "")
...
#   @rules - vector of additional equations user created
...
jl_generate_script <- function(species, 
  ...
  rules  <- sapply(rules, ConvertVarForJulia, USE.NAMES = FALSE)
